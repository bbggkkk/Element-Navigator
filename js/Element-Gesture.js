...
var mousedownEvt = function (event) {
    var _this = this;
...
HTMLElement.prototype.ungesture = function(){
...
HTMLElement.prototype.gesture = function ($event$) {
    ...
    this.gestureData = {};
    if($event === false){
        this.removeEventListener('mousedown', mousedownEvt);
        this.removeEventListener('touchstart', touchstartEvt);
    }else{
        this.addEventListener('mousedown', mousedownEvt, { passive : true });
        this.addEventListener('touchstart', touchstartEvt, { passive : true } );
    }
    // type.forEach(item => {
    // });
};
